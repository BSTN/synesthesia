  ...
  <transition name="fade">
    <div id="compare" @click="close()" v-if="open">
      <div id="frame" @click.stop>
        <md
          md="share.instructions"
          id="instructions"
          v-if="$store.state.profile.USERID"
        ></md>
        <md
          md="share.instructions.noid"
          id="instructions"
          v-if="!$store.state.profile.USERID"
        ></md>
        ...
        <a
          :href="downloadstring"
          class="button"
          download="my_synesthesia_testdata.json"
          >Download jouw data</a
        ><Br /><Br />

        <label v-if="$store.state.profile.SHARED"> Jouw unieke code: </label>
        <div id="unique" v-if="$store.state.profile.SHARED">
          {{ $store.state.profile.SHARED }}
        </div>

        <!-- doe eerst een test om je data te kunnen delen -->

        <label v-if="Object.keys(profiles).length > 0">{{
          $t("selecttocompare")
        }}</label>
        <div id="list" v-if="Object.keys(profiles).length > 0">
          <div
            ...
            :class="{ active: $store.state.shared.active === id }"
          >
        ...
        <button id="addnew" class="button" @click="add = true">
          {{ $t("addsharedresults") }}
        </button>
        ...
        <div id="add" v-if="add" @click="add = false">
          <div id="frame" @click.stop>
            ...
            <input
              ...
              v-model="sharedName"
            />
            <button id="addprofile" @click="addProfile()" class="button">
              ok
            </button>
          </div>
        </div>
      ...
      if (val) {
        ...
        this.$nextTick(() => {
          if (this.$refs.uploadfile)
            this.$refs.uploadfile.addEventListener(
              "change",
