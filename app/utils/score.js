...
function likert(testid, data) {
  ...
  const check = [
    ...
    testid + "5",
    testid + "6"
  ].filter(x => x in Object.keys(data));
...
function all(testconfig, questions) {
  ...
  each(questions, v => {
    // define name
    let name =
      typeof v.symbol === "string" ? v.symbol : JSON.stringify(v.symbol);
    ...
    if (!bundled[name])
      bundled[name] = { score: null, data: [], symbol: v.symbol };
  ...
  if (testconfig.type === "grapheme" || testconfig.type === "audio") {
    ...
    if (total !== undefined && count > 1 && !hasDoubles)
      total = total === 0 ? 0 : total / count;
